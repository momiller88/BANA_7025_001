...
After evaluating the results from the table, the only city we found to have an improper value was DUMBO. The proper value should of been Brooklyn. The code below allows us to fix this improper value. 

```{r}
#Change DUMBO to Brooklyn#
barstool_variable_name[barstool_variable_name$city == "DUMBO", ] <- "Brooklyn"
```
