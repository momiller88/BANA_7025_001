...
shinyApp(ui = fluidPage(
  selectInput("type_of_rank", "Type of Rank:", choices = sort(unique(barstool_for_table$type_of_rank)), multiple = FALSE),
  plotOutput("bar", height = 300, width = 800)
),
server = function(input, output) {
  selected_data <- reactive({
    req(input$type_of_rank)
    barstool_for_table %>% dplyr::filter(type_of_rank %in% input$type_of_rank)})
