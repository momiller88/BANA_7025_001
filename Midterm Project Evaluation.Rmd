...
We plan to uncover new information by combining the datafiniti and barstool datasets to answer the question about finding the top 5 pizza places by zip, price, and type of rating. Also, we plan to uncover new information by combining the datafiniti, barstool, and jared datasets in order to answer the question about what makes a pizza place receive a high rating. In all of these combinations, we hope to find matches between the datasets in order to answer these questions. We need to do more research in order to peform these techniques properly. 
...
### Combined Datasets
```{r}
#Combined Dataset for Shiny Table for NY Pizza Places#
bar_data_comb <- barstool_variable_name %>% 
                  inner_join(datafiniti_dup_removed, by = "address") %>%
                  filter(city.x == "New York City") %>%
                  dplyr::select("name.x", "address", "zip", "provider_rating", "review_stats_all_average_score", "review_stats_community_average_score", "review_stats_critic_average_score", "review_stats_dave_average_score", "price_range_min", "price_range_max") 
bar_data_comb_rank <- bar_data_comb %>%
                        mutate(prov_rat_rank = min_rank(-provider_rating)) %>%
                        mutate(rev_stats_all_avg_sco_rank = min_rank(-review_stats_all_average_score)) %>%
                        mutate(rev_stats_com_avg_sco_rank = min_rank(-review_stats_community_average_score)) %>%
                        mutate(rev_stats_crit_avg_sco_rank = min_rank(-review_stats_critic_average_score)) %>% 
                        mutate(rev_stats_dave_avg_sco_rank = min_rank(-review_stats_dave_average_score))
datatable(head(bar_data_comb_rank))
```

```{r}
bar_data_jared_comb <- barstool_variable_name %>%
                        inner_join(datafiniti_dup_removed, by = "name") %>% 
                        inner_join(jared2, by = c("name" = "place"))
bar_data_jared_comb
#Need to look at the tables for names and see if any need to be changed to match#                       
```
...
```{r}
#Doesn't work yet but want to create a table to show barstool data and select the type of rating you want to see
library(ggplot2)
library(shiny)
library(shinyWidgets)
barstool_tab <- barstool_for_table[ , c(1:3,6,8,7)]
head(barstool_tab)

shinyApp(
  ui = fluidPage(selectInput("zip", "Zip:", choices = sort(unique(barstool_tab$zip)), multiple = TRUE),
                 selectInput("price_level", "Price Level:", choices =  sort(unique(barstool_tab$price_level)), multiple = TRUE),
                 selectInput("type_of_rating", "Type of Rating:", choices = sort(unique(barstool_tab$type_of_rating)), multiple = FALSE), DT::dataTableOutput('tbl'), plotOutput("hist" , height = 300, width = 700)),
  server = function(input, output) {selected_data_tab <- reactive({
    req(input$zip)
    req(input$price_level)
    req(input$type_of_rating)
    barstool_tab %>% dplyr::filter(type_of_rating %in% input$type_of_rating & zip %in% input$zip & price_level %in% input$price_level)})
    output$tbl = DT::renderDataTable(
      selected_data_tab(), options = list(lengthChange = FALSE))
    output$hist <- renderPlot({
    ggplot(selected_data_tab(), aes(rating_score)) + geom_histogram()})
  })
```

```{r}
usa <- ggplot2::map_data("world", "usa")
shinyApp(ui = fluidPage(
  ...
  plotOutput("bar", height = 300, width = 800)
),
server = function(input, output) {
  selected_data <- reactive({
    req(input$type_of_rating)
    barstool_for_table %>% dplyr::filter(type_of_rating %in% input$type_of_rating)})
...
shinyApp(
  ui = fluidPage(selectInput("province", "Province:", choices = sort(unique(datafiniti_tab$province)), multiple = TRUE),
                 selectInput("price_type", "Price Type:", choices =  sort(unique(datafiniti_tab$price_type)), multiple = FALSE),
  ...
  server = function(input, output) {selected_data_tab_2 <- reactive({
    ...
    output$tbl = DT::renderDataTable(
      selected_data_tab_2(), options = list(lengthChange = FALSE))
    output$data_2 <- renderPlot({
    ggplot(selected_data_tab_2(), aes(price_range_value)) + geom_histogram()
  }
)
  }
)
