...
Then we are joining the two new dataframes on 'polla_qid' into a new dataframe named 'dfm'.
...
Now that we have a complete list of only the duplicated records we can sum the duplicated values voting stats. We do this by grouping the records by place and question and using the summarise() function we total the sum of each of the vote count columns. We then save this to the datafram 'jared_dups_combined'. In this dataframe we dropped the existing percentage values because they will need to be recalculated based on our new totals. 
...
```{r}
#Creates a new dataset for the items that we want evaluated by a histogram#
datafiniti_for_histogram <- datafiniti_dup_removed[ , c(6:9)]
#Creates the interactive histogram#
shinyApp(
  ui = fluidPage(
