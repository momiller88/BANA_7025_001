...
The code below creates the shiny table and histogram. In this shiny data table and histogram, a user should be able to select the province, price type, and price range value that they would like to evaluate for datafiniti. After selecting the results, the user would be able to see a data table and histogram of these results. The table will show the pizza place name, address, city, province, price type, and price range value. The table will be sorted in descending order by price range value. Please make the selections below in order to see the data table and histogram. You can select multiple provinces, price types, and price range values. 
...
shinyApp(
  ...
  server = function(input, output) {selected_data_tab_2 <- reactive({
    ...
    ggplot(selected_data_tab_2(), aes(price_range_value)) + 
        ...
        ggtitle("Histogram of Datafiniti Items Selected", subtitle = "Select the province, price type, and price range value you want to evaluate")})},
  options = list(height = 1200))
```
