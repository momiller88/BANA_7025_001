...
barstool_for_table <-  barstool_variable_name %>%
                        ...
                        gather(type_of_rank, rank, 7:11) %>%
                        arrange(rank)
                        
                        
                        

datatable(head(barstool_for_table, 50))
```
...
library(ggplot2)
library(shiny)
barstool_tab <- barstool_for_table[ , c(1:4,6:8)]

shinyApp(
ui = fluidPage(
  selectInput("zip", "Zip:", choices = sort(unique(barstool_tab$zip)), multiple = FALSE),
  selectInput("price_level", "Price Level:", choices =  sort(unique(barstool_tab$price_level)), multiple = FALSE),
  selectInput("type_of_rank", "Type of Rank:", choices = sort(unique(barstool_tab$type_of_rank)), multiple = FALSE),
  DTOutput('table')),
server =function(input, output) 
{
  output$tbl = renderDT(datatable(barstool_for_table))
})
```
...
ggplot(usa, aes(long, lat)) + 
   geom_polygon(aes(group=group)) + geom_point(data= barstool_for_table, aes(x = (longitude), y = latitude, colour = rating_score, shape = type_of_rank)) + coord_cartesian(xlim=c(-180, -50), ylim = c(20, 78))


shinyApp(ui = fluidPage(
  selectInput("type_of_rank", "Type of Rank:", choices = sort(unique(barstool_tab$type_of_rank)), multiple = FALSE),
  plotOutput("bar", height = 300, width = 700)
),
server = function(input, output) {
  output$bar <- renderPlot({
  map + ggplot(data = !!input$type_of_rank)+ geom_point(aes(colour = factor(input$type_of_rank)))})
}
)
```