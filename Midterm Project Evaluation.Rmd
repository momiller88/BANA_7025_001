...
We would say most people love pizza. Also, most people want to try a new pizza when they travel or move somewhere new. This is the problem we are hoping to help answer through our analysis. Yelp and other food review sites are great for finding new food locations to eat at, but what they lack is the ability to tell you upfront what is the actual price range of the restaurant. They only provide you a 5 scale rating of the price range, and this can be interpreted very differently based on the person. Also, they don't always visual their ratings in great way.

One of our goals for this project is to allow users to find the best pizza place for them based on zip code, price level, and type of rating in the barstool dataset. Also, another goal is to allow users to see where the top rated pizza places are on a map based on the type of rating selected from the barstool dataset. Also, a goal of ours is to allow users to see which zip codes has the highest and lowest average rating score based on the type of rating selected from the barstool dataset. Also, we are hoping to show users which pizza place received the highest rating based on the average of all the rankings from the barstool dataset. In adittion, our  is to allow users to find the best pizza place for them based on province and price type from the datafiniti dataset. Another goal of ours is to allow users to see where the cheapest pizza places are located on a map based on the price type they select from the datafiniti dataset. Also, we have a goal of showing users the average price value of pizza by province and price type from the datafiniti dataset. Also, we have a goal of showing users which pizza places have the lowest average price value based on all the price type values from the datafiniti dataset. Also, our goal is to find the top 10 pizza places that have a good rating in the jared data set. Also, it is to find the top 10 pizza places that a bad rating in the jared dataset. Finally, our last goal is allow a user to find the best pizza place for them based on zip, price range, and type of rating.

We are planning on addressing these goals through the use of shiny, data tables, histograms, and joining data tables. We believe that these final results will help the user find their ideal pizza place.

### Packages Required

#### **Packages**
The packages that are needed for this evaluation are the tidyverse, readr, dplyr, ggplot2, DT, shiny, pastecs, tibble, and mapdata. The tidyverse package contains multiple other packages that are useful with data manipulation and evaluation. You can find more information on the tidyverse package by clicking [here](https://tidyverse.tidyverse.org/). The readr package helps us import the datasets for this project. You can find more information on the readr package by clicking [here](https://cran.r-project.org/web/packages/readr/readr.pdf). The tidyr package helps us reorganize the datasets. You can find more information on the tidyr package by clicking [here](https://www.rdocumentation.org/packages/tidyr/versions/0.8.3). The ggplot2 package helps us create visualizations for these datasets. You can find more information on the ggplot2 package by clicking [here](https://cran.r-project.org/web/packages/ggplot2/ggplot2.pdf). The DT package helps us create data tables from the R output. You can find more information on the DT package by clicking [here](https://rstudio.github.io/DT/). The shiny package helps us create interactive data visualizations. You can find more information on the shiny package by clicking [here](https://cran.r-project.org/web/packages/shiny/index.html). The pastecs package helps us summarize the numeric values from the datasets. You can find more information on the pastecs package by clicking [here](https://www.rdocumentation.org/packages/pastecs/versions/1.3.21). The tibble package helps us add row.names to the character variable summary. You can find more information on the tibble package by clicking [here](https://tibble.tidyverse.org/reference/tibble-package.html). The mapdata package helped us with creating the map visualizations. You can find more information on the mapdatapackage by clicking [here](https://cran.r-project.org/web/packages/mapdata/index.html).  The first section of code allows you to install the packages. The second section of code allows you to load the packages into R. You need to run both of these sections of code in order to get these packages into R. 
...
datatable(head(barstool_variable_name, 10))
```
...
datatable(head(datafiniti_dup_removed, 10))
```
...
This first part allows users to find the best pizza place for them based on zip code, price, level, and type of rating. In this part, we first needed to edit the final dataset for barstool in order to make visualizations for it. This new dataset we created from the barstool dataset was to help us create a shiny table and histogram. In this new dataset, we need to use some of the orignial columns to the create the type of rating and rating score columns. Also, we needed to remove some columns that we didn't want in our data table. Below is the code to create the dataset. 
...
The code below creates the shiny table and histogram. In this shiny data table and histogram, a user should be able to select the zip, price level, and type of rating that they would like to evaluate for barstool. After selecting the results, the user would be able to see a data table and histogram of these results. The table will show the pizza place name, address, zip, price level, type of rating, and rating score. The table will be sorted in descending order by rating score. Please make the selections below in order to see the data table and histogram. You can select multiple zip codes and price levels. 

```{r}
#Creates shiny table and histogram#
shinyApp(
  ui = fluidPage(selectInput("zip", "Select the zip you want to evaluate:", choices = sort(unique(barstool_tab$zip)), multiple = TRUE),             selectInput("price_level", "Select the price level you want to evaluate:", choices =  sort(unique(barstool_tab$price_level)), multiple = TRUE),
...
This part of the analysis allows users to see where the pizza places are on a map and the rating score they had based on the type of rating selected. We used the barstool_for_table dataset from above to plot points on the map. We decided to use a shiny map to allow users to see where the top rate pizza places are. A user will be able to select type of rating they would like to see on the map. Below is the code to create this shiny map. 
...
After all of the analysis on the Barstool dataset we were able to derive several key insights. We noticed that not every zipcode had every price level. This would make sense considering that resturants would want to cater towards their clientelle and those in more affluent areas would be able to charge premium prices. From the map we were able to notice that the majority of our data was collected from areas in the northeast of the US, with heavy focus on the New York area. Selecting through the different ratings you will notice that the average community score is tends to higer than critics ratings. Where as, provider and overall average ratings tend to be more comparable across the board. Additionally, we provided summary tables for the top zipcodes based on rating. Finally, if you know where you would like to travel you can search the particular zipcode and see what rating it was given based on our datasets. From our own evaluations the top zipcodes based on provider ratings were 01801, 02115, 02563, 10461, 10550, 11003, 11231, 11238, 32250, and 33602. The top ten for overall average ratings were 11231, 10550, 60453, 11230, 02301, 11224, 33139, 02467, 02151, and 02368. The top ten based on community average score were 52241, 60453, 11231, 10550, 02301, 11230, 33480, 11224, 02151, and 33139. The top ten for critic's average ratings were 30342, 10280, 02128, 30326, 10038, 40202, 30305, 55417, 06338, and 32118. And, according to Dave's average scores the top ten zipcodes are 01902, 02467, 11230, 11231, 02184, 10550, 02072, 07110, 02564, and 10035. Finally, the leading pizza places based on average score are Serafina Broadway with 7.773, Jiannetto's Pizza Truck with 7.625, and Lucali - Henry with 7.574.

### Datafiniti Exploratory Data Analysis

#### **Datafiniti Exploratory Data Analysis Results**

The goal of this exploratory data analysis is to allow users to find the best pizza place for them based on province and price type. Another goal of ours is to allow users to see where the cheapest pizza places are located on a map based on the price type they select. Also, we have a goal of showing users the average price value of pizza by province and price type. Finally, we have a goal of showing users which pizza places have the lowest average price value based on all the price type values. 

This first part allows users to find best pizza place for them based on province and price type. We thought the best way to achieve this goal would be to create a shiny data table and histogram. Before we would create the shiny data table and histogram, we needed to create a data set for this data table and histogram. The code below allows us to create this new dataset. In this new dataset, we need to use some of the orignial columns to the create the price type and price range value columns. Also, we needed to remove some columns that we didn't want in our data table. Below is the code to create this new dataset.
...
The code below creates the shiny table and histogram. In this shiny data table and histogram, a user should be able to select the province and price type that they would like to evaluate for datafiniti. After selecting the results, the user would be able to see a data table and histogram of these results. The table will show the pizza place name, address, city, province, price type, and price range value. The table will be sorted in descending order by price range value. Please make the selections below in order to see the data table and histogram. You can select multiple provinces and price types. 

```{r}
#Creates shiny table and histogram#
shinyApp(
  ui = fluidPage(selectInput("province", "Select the province you want to evaluate:", choices = sort(unique(datafiniti_tab$province)), multiple = TRUE),             selectInput("price_type", "Select the price type you want to evaluate:", choices =  sort(unique(datafiniti_tab$price_type)), multiple = FALSE),
                 DT::dataTableOutput('tbl'), 
                 plotOutput("hist" , height = 300, width = 700)),
  server = function(input, output) {selected_data_tab_2 <- reactive({
    ...
    datafiniti_tab %>% 
      dplyr::filter(province %in% input$province & price_type %in% input$price_type)})
    output$tbl = DT::renderDataTable(
      selected_data_tab_2(), options = list(lengthChange = FALSE), caption = "This table shows the pizza places based on the selections above in order of lowest price range value.")
    output$hist <- renderPlot({
    ggplot(selected_data_tab_2(), aes(price_range_value)) + 
        geom_histogram() + 
        scale_x_continuous(name = "price range value") +
        ggtitle("Histogram of Datafiniti Items Selected", subtitle = "Select the province and price type you would like to evaluate through a histogram")})},
  options = list(height = 1200))
```

This part of the analysis allows users to see where the pizza places are on a map and the price range value they have based on the price type you selected. We used the datafiniti_map dataset from above to plot points on the map. We decided to use a shiny map to allow users to see where the cheapest pizza places are. A user will be able to select price type they would like to see on the map. Below is the code to create this shiny map.

```{r}
#Creates shiny map for datafiniti#
shinyApp(ui = fluidPage(
  ...
  server = function(input, output) {
    ...
    req(input$price_type)
      datafiniti_map %>% 
        dplyr::filter(price_type %in% input$price_type)})
    output$bar <- renderPlot({
      if (require("maps")) {
        data(us.cities)
        capitals <- subset(us.cities, capital == 2)
...
In this part, our goal is to allow users to see which province has the lowest price range value based on the price type selected. We decided that we thought we would achieve this goal through a shiny table. The first item we needed to do before we created the table was to create a dataset for it. The code below allows us to calculate the average price value by province and price type. Also, we arranged the average price value in ascending order. We thought this dataset would help us in creating the table to show which provinces had the lowest average price value based on the price type selected. Below is the code to create this dataset.

```{r}
#Create dataset to help us create a shiny table to show which provinces had the lowest average price value based on the price type selected#
datafiniti_state <- datafiniti_map %>%
  group_by(province, price_type) %>%
  summarise(average_price_value = mean(price_range_value, na.rm = TRUE)) %>% 
  arrange(average_price_value) 
```

The following code allows us to create the shiny table that was mentioned prior. Please make sure to select the price type you want to evaluate before you start evaluating the table.

```{r}
#Create shiny table to show which provinces had the lowest average price value#
shinyApp(
  ...
  output$tbl = DT::renderDataTable(
    selected_data_tab_3(), options = list(lengthChange = FALSE), caption = "This shows the average price value by province and price_type. This is sorted by the lowest average price value. Select the price type you want to evaluate.")},
  options = list(height = 700))
```

The final part we wanted to do for our exploratory analysis was to allow users to see which pizza place had the lowest average price across all the price types. The code below allows us to achieve this goal. In this code, we first need to group the data by name, address, and city. Next, we created a calculation to find the mean of all the prices. Finally, we sorted the data in descending order by average price. The last piece of code allows us to see the top 10 results for lowest average price for a pizza place.

```{r}
#Creates table of top ten places with the lowest average price#
datafiniti_price <- datafiniti_map %>%
  group_by(name, address, city) %>%
  summarise(average_price = mean(price_range_value, na.rm = TRUE)) %>%
  arrange(average_price)
datatable(head(datafiniti_price, 10))
```

#### **Summary of Datafiniti Exploratory Data Analysis Results**

After all of the analysis on the Datafiniti dataset we developed several tools that brought out some key insights. The first is the shiny table that allows us to filter pizza places based on state, price range max/min, and a select value for the max, or min, price. So, the user can mix and match any sort of combination to figure out the best pizza place in their area to fit their budget. The map allows us to visualize the most and least expensive places to get pizza. The majority of the most expensive pizza places tended to center around coastal regions and large cities. While the more rural locations tended to have a lower price cap. We were able to derive the most expensive states to buy pizza based on our data set, leading the top ten with Conneticut, South Dakota, Iowa, Alabama, Nebraska, Arizona, West Virginia, Maryland, Louisiana, and Oregon. Finally, our last table allows you to filter each pizza place based on their average price. The top ten places being: Crust Stone Oven Pizza, Oregano, Apizza Scholls, Barbaro, Buca di Beppo Italian Resturant, Dough Pizzeria Napoletana, La Montanara Resturant, Papa's Pizza, Spin, and Stella Barra Pizzeria.

### Jared Exploratory Data Analysis

#### **Jared Exploratory Data Analysis Results**

The goal of this exploratory data analysis is to find the top 10 pizza places that have a good rating. Also, it is to find the top 10 pizza places that a bad rating. We thought that a data table would best illustrate these findings. Before we made the table, we needed to create a calculation to get the good rating. We found the good rating by summing up the percent good and the percent excellent. We thought that these two selections would determine whether a pizza place was really good. Also, we needed to create a calculation to sum up the percent poor and the percent never again. We thought that these two selections would determine whether a pizza place was really bad. Next, we greated new datasets based of these selections so we could create two separate tables. Below is the code to create this dataset and data table. 

```{r}
#Create good and bad rating#
...
#Create jared good dataset#
jared_good <- jared_new_calc %>%
  arrange(desc(good_rating))
#Create jared bad dataset#
jared_bad <- jared_new_calc %>%
  arrange(desc(bad_rating))
#Create jared data table to show the top 10 pizza places with highest percentage for good#
datatable(head(jared_good, 10), caption = "This table shows the top 10 jared items that have a good rating.")
#Create jared data table to show the top 10 pizza places with the highest percentage for bad#
datatable(head(jared_bad, 10), caption = "This table shows the top 10 jared items that have a bad rating.")
```

#### **Summary of Jared Exploratory Data Analysis**

After evaluating the good table, we found that the top ten good places for pizza are Patsy's, Dona Bella, Williamsburg Pizza, Tappo, Artichoke Basille's Pizza, Rocky's, Naples 45, Fiore's, Bleecker Street Pizza, and Arturo's. After evaluating the bad table, we found that the top ten bad places for pizza are Ben's of SoHO 14th Street, Kiss My Slice, Frank's Express Pizza, Nonna's LES Pizza, Previti Pizza, Steve's Pizza, Highline Pizza, Prince Street Pizza, and Champion Pizza. We found these values by looking at which values had the largest value for good or bad rating.

### Datafiniti and Barstool Combination Exploratory Data Analysis

#### **Datafiniti and Barstool Combination Exploratory Data Analysis**

The goal of this exploratory data analysis is to allow a user to find the best pizza place for them based on zip, price range, and type of rating. We thought a shiny data table and histogram would best illustrate these findings. The first thing we did was combine the datafiniti and barstool datasets through an inner join. We thought address would be good to do the inner join since it is likely a unique value. Next, we selected the columns we wanted from both datasets and we arranged them by rating score. Finally, we selected the columns we wanted included in the data table. Below is the code to create this dataset. 

```{r}
#Creates dataset for shiny data table and histogram#
...
datatable(head(data_tab))
```

The code below allows creates the shiny data table and histogram that was mentioned above. Please make sure to select zip, price range, and type of rating.

```{r}
#Create shiny data table and histogram#
shinyApp(
...
#### **Summary of Combined Datafiniti and Barstool Exploratory Data Analysis**

After completing this exploratory analysis, we found that there weren't many matches between the datafiniti and barstool. We were still able to get a good amount that it would allow a user to use this table to search for their ideal pizza place. Also, we discovered from this join that most of the similar results were from NY. 

### Summary Evaluation

#### **Summary Evaluation**

We had a variety of goals we wanted to solve at the beginning of this project. They are the following. One of our goals for this project is to allow users to find the best pizza place for them based on zip code, price level, and type of rating in the barstool dataset. Also, another goal is to allow users to see where the top rated pizza places are on a map based on the type of rating selected from the barstool dataset. Also, a goal of ours is to allow users to see which zip codes has the highest and lowest average rating score based on the type of rating selected from the barstool dataset. Also, we are hoping to show users which pizza place received the highest rating based on the average of all the rankings from the barstool dataset. In adittion, our  is to allow users to find the best pizza place for them based on province and price type from the datafiniti dataset. Another goal of ours is to allow users to see where the cheapest pizza places are located on a map based on the price type they select from the datafiniti dataset. Also, we have a goal of showing users the average price value of pizza by province and price type from the datafiniti dataset. Also, we have a goal of showing users which pizza places have the lowest average price value based on all the price type values from the datafiniti dataset. Also, our goal is to find the top 10 pizza places that have a good rating in the jared data set. Also, it is to find the top 10 pizza places that a bad rating in the jared dataset. Finally, our last goal is allow a user to find the best pizza place for them based on zip, price range, and type of rating.

We are addressed these goals through the use of shiny, data tables, histograms, and joining data tables. If you would like to see more information, look at the prior exploratory tabs. Also, we had a variety of results from addressing these goals. You can see our analysis and findings in the prior exploratory tabs.

Based on our analysis of these datasets we believe any user can gain a better understanding of the varied pizza places across the US. They should be able to filter and sort based on their location, or planned destination, to find pizza places in their price range. They should be able to determine which places are a "Must Try!" and which places are "Avoid at ALL Cost!". Our analysis and tools provide a lot of varied insights for many different types of users. However, with additional datapoints and clarity around the data liniage we my have been able to derive more. We were not able to merge all three datasets in an effective manner that would result in enough records to run our regression analysis. We wanted to highlight key attributes that lead to the highest rated pizza place by both Barstool, Critics, and the Community. But, we simply didn't have enough information. Additionally, with some of our visualizations you may be limited to secific areas. Not every state is represented and there is a clear skew towards the north eastern region of the country. This could ultimately lead to bias in our results based on limited records in other states.
