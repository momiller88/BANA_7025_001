...
datatable(head(barstool_rate, 10), caption = "This shows the top 10 pizza places based on highest average score.")
```
...
The final part we wanted to do for our exploratory analysis was to allow users to see which pizza place had the lowest average price across all the price types. The code below allows us to achieve this goal. In this code, we first need to group the data by name, address, and city. Next, we created a calculation to find the mean of all the price types. Finally, we sorted the data in descending order by average price. The last piece of code allows us to see the top 10 results for pizza places that have the lowest average price.

```{r}
#Creates dataset and data table to see the top 10 pizza places for lowest average price#
datafiniti_price <- datafiniti_map %>%
  group_by(name, address, city) %>%
  summarise(average_price = mean(price_range_value, na.rm = TRUE)) %>%
  arrange(average_price)
datatable(head(datafiniti_price, 10), caption = "This table shows the top 10 pizza place for the lowest average price.")
```
