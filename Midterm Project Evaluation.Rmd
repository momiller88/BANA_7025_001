...
We would say most people love pizza. Also, most people want to try a new pizza when they travel or move somewhere new. This is the problem we are hoping to help answer through our analysis. Yelp and other food review sites are great for finding new food locations to eat at, but what they lack is the ability to tell you upfront what is the actual price range of the restaurant. They only provide you a 5 scale rating of the price range, and this can be interpreted very differently based on the person. Our goal was to show you the top pizza places by price range, zip, and type of rating in an easy to use interactive format. Our analysis provides you with a more concrete number of the price range each resturant. We have created interative datatables, maps, and histograms of the the top rated pizza places by zip code, price range, and type of review. This analysis was all done using a shiny application, because shiny apps give us the ability to make our visualizations interactive. We combined the barstool and datafiniti datasets, so we can have reviews and price ranges in a single view. And, the table will contains the name, address, price range, and rating of each resturant. This table will help the user find the best pizza place they can afford to eat whenever they are traveling or want to try a new pizza place.
...
The goal of this part is to find the top 10 pizza places that have a good rating. Also, it is to find the top 10 pizza places that a bad rating. We thought that a data table would best illustrate these findings. Before we made the table, we needed to create a calculation to get the good rating. We found the good rating by summing up the percent good and the percent excellent. We thought that these two selections would determine whether a pizza place was really good. Also, we needed to create a calculation to sum up the percent poor and the percent never again. We  
...
#### **Datafiniti and Barstool Combination Exploratory Data Analysis Results**
